...
| [docs/tracking-plan.md](docs/tracking-plan.md)                                       | Event-tracking (instrumentation) dictionary: naming convention, per-event registry, base props, A/B experiment convention                                                                                           |
...
make dev             # backend (:3100 REST / :50051 gRPC) + frontend (:5173), watch mode
# in a second terminal, once `make dev` is up:
make seed            # demo users (incl. admin@example.com, role: editor) + sample analytics events
make promote-admin EMAIL=admin@example.com   # seed creates an editor — this grants the admin role
```
...
**gRPC-first**: every business operation is a gRPC method (`proto/`), consumed by the browser via gRPC-Web through Envoy. **REST (`/api/v1`) fills the gaps only** — file uploads, OAuth callbacks, webhooks, health/metrics. **WebSocket (Socket.io)** handles realtime notifications and the live event feed. Details in [docs/architecture.md](docs/architecture.md).
...
│   └── temporal-worker/   Legacy standalone worker (unused — the live worker runs
│                          in-process in the backend: apps/backend/src/infrastructure/temporal/)
...
| Layer                      | Choice                                                                                                                                                                            | Why                                                                                                                                                               |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Frontend — helm**        | React 18 + Vite + Tailwind v4/shadcn, TanStack Query + Zustand, react-hook-form + zod, i18next (en/zh), PWA                                                                       | The logged-in **admin console** (CSR SPA, `noindex`); also wrapped by the mobile (Capacitor) and desktop (Tauri) shells. Conventions in docs/project-structure.md |
| **Frontend — harbor**      | Next.js 15 (App Router, SSR/SSG)                                                                                                                                                  | The public **marketing/content site** — full SEO (Metadata API, `robots.ts`/`sitemap.ts`). Split rationale: docs/web-quality.md                                   |
...
| **Async**                  | Pulsar (events, behind MessageBrokerPort, per-topic `<topic>-DLQ`), BullMQ (jobs + `dead-letter` DLQ), Temporal (durable workflows, in-process worker), Flink (stream processing) | Distinct roles: fact broadcast vs job vs saga vs stream                                                                                                           |
| **Reliability**            | Outbox, idempotency keys, circuit breaker, replay-safe handlers, feature flags                                                                                                    | See "Reliability patterns" below                                                                                                                                  |
| **Auth & policy**          | JWT + Passport (OAuth2 Google/GitHub), RBAC, OPA (Rego), multi-tenancy (MongoDB and ClickHouse, both transports), Vault (secrets + Transit encryption)                            | AuthN in-app, authZ externalized, secrets never in code                                                                                                           |
...
| **OPA**                | Externalised policy         | Role-to-action permissions defined in Rego (`infra/opa/authz.rego`). NestJS asks OPA's REST API `POST /v1/data/authz/allow` on every authenticated gRPC call — reads and writes alike — so policy changes need no code deploy. OPA calls are wrapped in a circuit breaker: 5 consecutive failures open it for 30 s, and an open circuit denies, so an OPA outage cannot become an authorization bypass.                                                                                                                                                                                                                                                                                                 |
| **RBAC**               | Role-based access           | Three roles (`admin`, `editor`, `viewer`) carried in the JWT, with role-to-action permissions in Rego rather than in code. Enforcement is imperative rather than decorator-based, so a permission check is visible at the call site. Details and the full permission matrix: the RBAC row in the security table below.                                                                                                                                                                                                                                                                                                                                                                                  |
| **Outbox**             | Guaranteed event delivery   | Both user and analytics writes use the outbox pattern. Each write wraps the business document + outbox row in a single MongoDB transaction. A relay polls PENDING rows every 5 s and publishes to Pulsar (multi-topic: `user-events` and `analytics-events`). A publish error marks the row FAILED with an explicit `nextAttemptAt` (exponential backoff, base 5 s, doubling per attempt); a 60 s sweep requeues rows that are due and moves rows past `MAX_ATTEMPTS` (5) to the terminal **DEAD** status, whose backlog is logged for alerting. At-least-once delivery is guaranteed even if the process crashes mid-request.                                                                          |
...
| **DLQ + retry**        | Permanent failure isolation | Two independent dead-letter paths. **BullMQ:** jobs that exhaust all retry attempts are routed to the `dead-letter` queue — visible in Bull Board, and admins can replay any DLQ job back to its source queue. **Pulsar:** every consumer created by `PulsarBrokerAdapter.subscribe()` declares a dead-letter policy (`maxRedeliverCount: 5`, `deadLetterTopic: <topic>-DLQ`) plus a 5 s nack-redelivery delay and a 60 s ack timeout — a poison message is parked on `<topic>-DLQ` instead of being redelivered forever and blocking the subscription. Prometheus scrapes the broker directly, so `pulsar_msg_backlog{topic=~".*-DLQ"}` raises `PulsarDeadLetterBacklog` / `PulsarDeadLetterRateHigh`. |
...
| **Multi-tenancy**                | Every user document carries a `tenantId`. `UserRepository` appends `{ tenantId }` to every query — users from different tenants are completely invisible to each other. `TenantMiddleware` extracts `tenantId` from the JWT or `X-Tenant-ID` header on every HTTP request. Header override is blocked when a JWT is present, preventing tenant-escalation via header injection. Compound unique index on `(email, tenantId)` allows the same email across tenants. `GrpcTenantInterceptor` is registered as a global `APP_INTERCEPTOR` and binds `TenantContext` for **both transports** before any handler runs — from the JWT `tenantId` claim in the gRPC authorization metadata, and from `req.tenantId` (already resolved by `TenantMiddleware`) for HTTP. Scope of the isolation: MongoDB read/write paths AND the ClickHouse analytics tables. Every `logs.*` table carries `tenant_id` as the leading `ORDER BY` column, so a tenant-filtered read skips other tenants' granules instead of scanning and filtering them; `logs.analytics_minutely_agg` groups by `tenant_id` in its materialized view, and every analytics/tracking query passes the tenant from the verified JWT (never from a request field). |
| **RBAC**                         | Role checks are imperative OPA queries rather than decorators: `GrpcAuthzService.assert()` is the single gate for both gRPC controllers, and `assertCanManageRoles()` covers the REST roles endpoints. All call `OpaService.allow()` against `infra/opa/authz.rego`. Permissions on `user` are `create`, `read`, `list`, `update`, `delete`, `manage_roles`; on `analytics` they are `read` and `write`. **`list` is separate from `read` and admin-only**: registration is public and self-registered accounts default to `editor`, so an enumeration permission folded into `read` would let one free signup dump every user's email. `read` returns a single record and the controller additionally requires owner-or-admin. `delete`, `manage_roles` and writing a user's `status` are admin-only — `status` gates sign-in, so an owner-writable status would let a suspended account restore itself. Roles are a schema default, never taken from request data. The first admin is granted out-of-band with `make promote-admin EMAIL=<email>`; after that, admins use the roles endpoint.                                                                                                                         |
| **OPA**                          | Every authenticated gRPC method — the user reads and mutations, and the whole analytics tier — asks OPA's REST API whether the caller's roles allow the requested action on the requested resource. Policy lives in `infra/opa/authz.rego`: edit the Rego and restart OPA, no code deploy. Every failure path (timeout, non-2xx, open circuit) returns deny.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| **@nestjs/throttler**            | Two named throttlers, both per source IP and both configured from env: `default` (`RATE_LIMIT_DEFAULT`, 100 req / `RATE_LIMIT_TTL_MS`) applies everywhere, and `auth` (`RATE_LIMIT_AUTH`, 10 req) applies to the credential routes — login, refresh and both OAuth callbacks. The values are env-driven because a browser E2E run or a load test drives many real sign-ins from one IP and needs the credential limit raised (`make test-e2e` sets `E2E_RATE_LIMIT_AUTH`). The `auth` throttler is scoped to `/api/auth/*` so it cannot cap `/api/health` or `/api/metrics` and break liveness probes and scraping. `gRPC AuthService/Login` is rate-limited separately, 10 attempts / 60 s per email via a Redis counter (`grpc:login:rl:{email}`), because the throttler only sees HTTP.                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
```bash
pnpm dev          # backend + frontend concurrently (recommended)
...
# The live workflow is userOnboardingWorkflow (durable follow-up timer):
#   definition apps/backend/src/infrastructure/temporal/workflows.ts
#   run by the in-process worker (infrastructure/temporal/temporal.worker.ts)
#   started fire-and-forget from modules/user/processors/user.processor.ts
#   read back via GET /api/workflows/onboarding (Stack view + Users badges)
make wf-demo                           # end-to-end demo (CRASH=1 for crash recovery)
...
**TenantContext** is a **singleton over `AsyncLocalStorage`** (`src/common/tenant/tenant.context.ts`) — not a request-scoped provider. It is injectable into ordinary singletons (`UserService`, `UserRepository`, the CQRS handlers) without dragging the whole dependency chain into request scope, and any service can read `tenantCtx.tenantId` without threading `tenantId` through every signature.

The store is bound **once at the transport boundary** by `GrpcTenantInterceptor`, registered as a global `APP_INTERCEPTOR`, which wraps the handler in `tenantCtx.run(tenantId, …)` for **both transports**:

```
gRPC : JWT `tenantId` claim read from the Authorization metadata header
HTTP : `req.tenantId`, already resolved and validated by TenantMiddleware
both : falls back to DEFAULT_TENANT ('default')
```

Outside a request — cron jobs, message consumers, startup — there is no store, so `tenantCtx.tenantId` returns `'default'`. Those paths **must** pass a tenant explicitly when they touch tenant-scoped data.

> **The Flink job is tenant-aware too.** `flink/` consumes the same analytics
> topic on its own Pulsar subscription, resolves `tenant_id` from the outbox
> envelope (`AppEvent`), and writes to its **own** table
> `logs.analytics_events_flink` (`flink/sql/init-clickhouse.sql`). It has a
> separate table because both consumers receive every message and MergeTree
> does not deduplicate: sharing one table would double every count the console
> reads whenever the job is running. As it stands the job is safe to submit at
> any time, and querying both tables compares the in-process pipeline against
> the streaming one.

> **Upgrading an existing ClickHouse database.** `tenant_id` is the leading key column of `logs.analytics_events`, `logs.analytics_minutely_agg` and `logs.user_events`, and ClickHouse cannot add a leading key column in place. A database created before that must be migrated once with `infra/clickhouse/migrations/001-add-tenant-id.sql`; a fresh install gets the right schema from `init-*.sql` and needs nothing.
...
OutboxRelay (every 5 s)
  └─ acquire Redis distributed lock (NX, 8 s TTL) — skips if another instance holds it
       └─ fetch PENDING rows (batch 50)
            └─ PulsarClient.send()    →  mark DISPATCHED
            └─ on failure             →  mark FAILED + attempts++ and set
                                         nextAttemptAt = now + 5 s * 2^(attempts-1)
  └─ release lock (Lua atomic check-and-delete)

OutboxRelay (every 60 s)
  └─ requeueFailed(5)        → FAILED rows with attempts < 5 AND nextAttemptAt <= now
                               → back to PENDING (nextAttemptAt cleared)
  └─ deadLetterExhausted(5)  → FAILED rows with attempts >= 5 → DEAD (terminal)
  └─ countDead()             → logs the DEAD backlog
  └─ backlogByStatus()       → tropis_outbox_rows{status="pending|failed|dead"} gauge
```

Statuses: `PENDING` → `DISPATCHED` on success; `PENDING` → `FAILED` on a publish error, then back to `PENDING` once the exponential backoff (`nextAttemptAt`, base 5 s, doubling per attempt) elapses; `FAILED` → **`DEAD`** once `attempts` reaches `MAX_ATTEMPTS` (5). `DEAD` is terminal — the relay never retries it again. It exists so an exhausted row is explicit and countable instead of sitting in `FAILED` forever, silently lost; inspect the backlog with `make outbox-status`. The backlog is also exported as `tropis_outbox_rows` and alerted on (`OutboxDeadRows`, `OutboxFailedBacklogSustained`, `OutboxPendingBacklogGrowing`), and the relay's locked poll writes a heartbeat gauge that fires `OutboxRelayStalled` if nothing has relayed for a minute.
...
`packages/shared` (`@tropis/shared`) — a compiled TypeScript library imported by both the backend and frontend. Code lives here only when both sides must agree on a contract (event contracts, error codes, the tracking dictionary; it also carries some response-envelope and pagination types that no code imports yet) — promotion rules in [docs/project-structure.md](docs/project-structure.md) (three-tier placement).
...
NestJS exposes `/api/metrics` (Prometheus text format) via `@willsoto/nestjs-prometheus`. Custom metrics are registered in one registry (`modules/metrics/metrics.module.ts`, names in `metrics.constants.ts`):
...
- `tropis_outbox_rows` — gauge labelled `status` (`pending` / `failed` / `dead`), the outbox backlog
- `tropis_outbox_relay_last_poll_timestamp_seconds` — gauge, heartbeat of the relay's locked poll
- `tropis_tracking_duplicate_events_dropped_total` — counter, events rejected by the ingest dedup claim

The two outbox gauges are refreshed from the relay's own scheduled ticks (one grouped query a minute for the backlog, one gauge write per locked poll for the heartbeat), so reliability visibility costs nothing on the request path. Prometheus also scrapes the Pulsar broker (`pulsar:8080`) for topic-level series, which is what makes the `<topic>-DLQ` dead-letter topics alertable. Alert rules for all of it live in `infra/prometheus/alerts.yml`.
...
| **CORS_ORIGIN** | `http://localhost:5173`        | Your frontend origins, comma-separated. The packaged clients' custom-scheme origins are constants and are always allowed — see `docs/multi-platform.md` |
...
- `CORS_ORIGIN=https://yourdomain.com` → your real frontend origin(s), comma-separated
