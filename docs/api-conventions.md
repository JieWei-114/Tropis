...
| -------------- | --------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Surfaces       | REST `/api/v1/*` + gRPC `app.<domain>.v1` (gRPC-Web via Envoy)  | gRPC `app.<domain>.internal.v1` only                                                                                                                                                                                                |
...
- Called by a browser, mobile app, or third-party server → **public** (`app.<domain>.v1` / `/api/v1/*`).
- Called only by our own services (trusted lookups, bulk/admin operations, data that must never cross the trust boundary, e.g. `GetUserByEmail` without a user JWT) → **internal** (`app.<domain>.internal.v1`).
