...
| WebSocket (/ws)    | ws-listen (devtools/)        | `make ws-listen [TOKEN=<jwt>]` — prints every Socket.io event live (auto-logs-in as the seeded admin)                    |
...
| Temporal           | Temporal Web UI              | `make up-tools` → http://localhost:8233                                                                                  |
