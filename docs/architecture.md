...
| **Frontend — helm** (后台)   | `frontend/helm/`                  | Logged-in **admin console**. React 18 + Vite SPA (CSR), `noindex`. Talks gRPC-Web through Envoy; Socket.io for realtime. Ships as a static nginx image                                                                                  |
| **Frontend — harbor** (前台) | `frontend/harbor/`                | Public **marketing/content site**. Next.js App Router (SSR/SSG), full SEO. Ships as a standalone Node server                                                                                                                            |
...
3. **Pulsar** — the cross-service event bus. Consumers must tolerate unknown fields (add-only payload evolution; see `docs/api-versioning.md`).
