...
const { sendEmail, sendPushNotification, recordDelivery } = proxyActivities<NotificationActivities>({
...
export async function notificationWorkflow(input: NotificationWorkflowInput): Promise<void> {
  if (input.delayMs) {
  ...
  await Promise.all([
    ...
    sendPushNotification({ userId: input.userId, title: input.subject, body: input.body }),
  ]);

  await recordDelivery({ userId: input.userId, sentAt: new Date().toISOString() });
}
