...
import { registerAndLogin, uniqueId } from './helpers';

test.describe('analytics', () => {
  // Events travel MongoDB → Pulsar → processor → ClickHouse → SSE/stats;
  ...
  test('firing an event updates the live feed and stats', async ({ page }) => {
    ...
    await fire();
    await expect(page.locator('.fire-status')).toContainText('page_view', { timeout: 20_000 });

    // Live feed (SSE) should eventually show our event; re-fire periodically
    // in case the first one raced the stream subscription.
